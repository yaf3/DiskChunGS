...
int main() {
    try {
        // Configuration - you can easily change resolution for speed/quality tradeoff
        cv::Size model_resolution(1280, 384);  // Options: (192,144), (320,240), (480,360), (640,480), (832,624), (1024,768)
        
        // KITTI camera configuration with focal length scaled for model resolution
        float baseline = 0.53716f;  // KITTI baseline in meters
        float kitti_focal_length = 721.5377f;  // KITTI focal length for 1242x375 images
        // float scaled_focal_length = kitti_focal_length * model_resolution.width / 1242.0f;  // Scale for model resolution
        
        float max_distance = 80.0f;  // KITTI scenes can be quite far
        
        // Initialize model
        // std::string model_path = "./models/fast_acvnet_plus_onnx_no_gridsample/fast_acvnet_plus_kitti_2015_opset11_" + 
        ...
        std::cout << "Loading FastACVNet model from: " << model_path << std::endl;
        std::cout << "Model resolution: " << model_resolution.width << "x" << model_resolution.height << std::endl;
        // std::cout << "Scaled focal length: " << scaled_focal_length << " pixels" << std::endl;
        
        FastACVNet depth_estimator(model_path);
        ...
        if (!left_img.empty() && !right_img.empty()) {
            ...
            // Estimate depth
            cv::Mat depth_map = depth_estimator.estimate_metric_depth(left_img, right_img, kitti_focal_length, baseline);
            ...
            // cv::Mat model_disparity = depth_estimator.get_disparity_map(false);      // Model resolution
            // cv::Mat original_disparity = depth_estimator.get_disparity_map(true);   // Original image resolution
            // cv::Mat original_depth = depth_estimator.get_depth_map(true);           // Original image resolution
            
            // std::cout << "Model resolution disparity: " << model_disparity.size() << std::endl;
            // std::cout << "Original resolution disparity: " << original_disparity.size() << std::endl;
            ...
            //     cv::addWeighted(left_img, 0.6, color_depth, 0.4, 0, combined_overlay);
            // } else {
            ...
            // cv::imwrite("kitti_raw_disparity.exr", original_disparity);  // Save raw disparity as EXR
            // cv::imwrite("kitti_raw_depth.exr", original_depth);          // Save raw depth as EXR
            
            // std::cout << "\nResults saved:" << std::endl;
            // std::cout << "- kitti_depth_overlay.png (visualization)" << std::endl;
            // std::cout << "- kitti_disparity_comparison.png (comparison)" << std::endl;
            // std::cout << "- kitti_depth_map.png (depth only)" << std::endl;
            // std::cout << "- kitti_raw_disparity.exr (raw disparity data)" << std::endl;
            // std::cout << "- kitti_raw_depth.exr (raw depth data)" << std::endl;
