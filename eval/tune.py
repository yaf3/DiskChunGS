...
def read_psnr(result_dir):
    # Find the most recent shutdown directory
    shutdown_dirs = list(Path(result_dir).glob("*_shutdown"))
    if not shutdown_dirs:
        return -float('inf')
    latest_dir = max(shutdown_dirs, key=lambda x: int(x.name.split('_')[0]))
    
    # Read PSNR values
    psnr_file = latest_dir / "psnr.txt"
...
def objective(trial):
    ...
    hyperparams = {
        ...
        'Camera.z_near': trial.suggest_float('z_near', 0.5, 4.0),
        'Camera.z_far': trial.suggest_float('z_far', 30.0, 100.0),
        'Monocular.inactive_geo_densify_max_pixel_dist': trial.suggest_float('inactive_geo_densify_max_pixel_dist', 0.5, 2.0),
        
        # Mapper parameters
        'Mapper.min_num_initial_map_kfs': int(trial.suggest_int('min_num_initial_map_kfs', 12, 30)),
        ...
        'Optimization.position_lr_init': trial.suggest_float('position_lr_init', 1e-5, 1e-2, log=True),
        ...
        'Optimization.position_lr_max_steps': int(trial.suggest_int('position_lr_max_steps', 1000, 60000)),
    ...
    trial_config_path = f"cfg/gaussian_mapper/Stereo/KITTI/KITTI_trial_{trial.number}.yaml"
    with open(trial_config_path, 'w') as f:
        f.write(yaml_content)
    
    # Run the program
    result_dir = f"results/kitti/00_trial_{trial.number}"
    try:
        subprocess.run([
            "./bin/kitti_stereo",
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        storage="sqlite:///kitti_hypertune_rt.db",
