...
void ADAM::adamUpdate(float* param,
  ...
  adamUpdateCUDA<<<(cnt + 255) / 256, 256>>>(param, param_grad, exp_avg,
                                             exp_avg_sq, tiles_touched, lrs, b1,
                                             b2, eps, N, M, per_primitive_lr);
}

__global__ void adamUpdateBasicCUDA(float* __restrict__ param,
                                    const float* __restrict__ param_grad,
                                    float* __restrict__ exp_avg,
                                    float* __restrict__ exp_avg_sq,
                                    const float lr,
...
void ADAM::adamUpdateBasic(float* param,
                           const float* param_grad,
                           float* exp_avg,
                           float* exp_avg_sq,
                           const float lr,
                           ...
                           const uint32_t N) {
  if (N <= 256) {
    adamUpdateBasicCUDA<<<1, N>>>(param, param_grad, exp_avg, exp_avg_sq, lr,
                                  b1, b2, eps, N);
  } else {
    adamUpdateBasicCUDA<<<(N + 255) / 256, 256>>>(
        param, param_grad, exp_avg, exp_avg_sq, lr, b1, b2, eps, N);
  }
}