...
__device__ void load_into_shared(float pixels[SY][SSX],
                                 const float* inp,
                                 const int CH,
                                 const int H,
                                 const int W,
...
__device__ void do_separable_conv_x(float pixels[SY][SSX],
                                    float opt[CY][CCX],
                                    int H,
                                    int W,
                                    bool sq = false) {
...
__device__ float do_separable_conv_y(float pixels[CY][CCX],
                                     int H,
                                     int W,
                                     bool sq = false) {
  auto block = cg::this_thread_block();
...
fusedssim(float C1,
  ...
  torch::Tensor dm_dmu1 =
      train ? torch::zeros_like(img1).contiguous() : torch::empty(0);
  torch::Tensor dm_dsigma1_sq =
      train ? torch::zeros_like(img1).contiguous() : torch::empty(0);
  torch::Tensor dm_dsigma12 =
      train ? torch::zeros_like(img1).contiguous() : torch::empty(0);
  fusedssimCUDA<<<grid, block>>>(
      H, W, CH, C1, C2, img1.contiguous().data<float>(),
...
torch::Tensor fusedssim_backward(float C1,
  ...
  fusedssim_backwardCUDA<<<grid, block>>>(
      ...
      dL_dimg1.contiguous().data<float>(), dm_dmu1.contiguous().data<float>(),
      dm_dsigma1_sq.contiguous().data<float>(),
      dm_dsigma12.contiguous().data<float>());
