...
RasterizeTrianglesCUDA(const torch::Tensor& background,
  ...
  torch::Tensor radii =
      torch::full({P}, 0, triangles_points.options().dtype(torch::kInt32));
  ...
  const int total_nb_points = num_points_per_triangle[P - 1].item<int>() +
                              cumsum_of_points_per_triangle[P - 1].item<int>();
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        total_nb_points, sh.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_others, radii, geomBuffer,
                         binningBuffer, imgBuffer, scaling, density_factor,
                         max_blending);
}
...
RasterizeTrianglesBackwardCUDA(
  ...
  torch::Tensor dL_dtriangle =
      torch::zeros({total_nb_points, 3}, triangles_points.options());
  ...
  torch::Tensor dL_dcolors =
      torch::zeros({P, NUM_CHANNELS}, triangles_points.options());
  ...
  torch::Tensor dL_dsigma_factor =
      torch::zeros({P, 1}, triangles_points.options());
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::backward(
        ...
        dL_dsigma_factor.contiguous().data_ptr<float>(), debug);
  }

  return std::make_tuple(dL_dtriangle, dL_dsigma, dL_dcolors, dL_dopacity,
                         dL_dsh, dL_dmeans2D);
}
