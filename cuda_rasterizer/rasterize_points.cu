...
RasterizeTrianglesCUDA(const torch::Tensor& background,
  ...
  torch::Tensor radii =
      torch::full({P}, 0, vertices.options().dtype(torch::kInt32));

  torch::Tensor proba_existence = torch::full({P}, 0.0, float_opts);
  torch::Tensor was_rendered =
      torch::full({P}, 0, vertices.options().dtype(torch::kInt32));
  ...
  const int total_nb_points =
      P * 3;  // FOR EACH TRIANGLE, WE CAN HAVE 3 NORMALS, OFFSETS,...
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        was_rendered.contiguous().data_ptr<int>(), debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, was_rendered,
                         geomBuffer, binningBuffer, imgBuffer, scaling,
                         max_blending);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
RasterizeTrianglesBackwardCUDA(const torch::Tensor& background,
                               const torch::Tensor& vertices,
                               const torch::Tensor& triangles_indices,
                               const torch::Tensor& vertex_weights,
                               const float sigma,
  ...
  torch::Tensor dL_dnormals =
      torch::zeros({total_nb_points, 3}, vertices.options());
  torch::Tensor dL_doffsets =
      torch::zeros({total_nb_points, 3}, vertices.options());

  torch::Tensor dL_dcolors =
      torch::zeros({V, NUM_CHANNELS}, vertices.options());
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::backward(
        P, V, degree, M, R, background.contiguous().data_ptr<float>(), W, H,
        vertices.contiguous().data_ptr<float>(),
        triangles_indices.contiguous().data_ptr<int>(),
        vertex_weights.contiguous().data_ptr<float>(), sigma, total_nb_points,
        sh.contiguous().data_ptr<float>(),
        ...
        dL_dvertice_depth.contiguous().data_ptr<float>(), debug);
  }

  return std::make_tuple(dL_dvertices3D, dL_dvertice_weight, dL_dcolors,
                         dL_dsh);
}
...
std::tuple<torch::Tensor, torch::Tensor> ComputeRelocationCUDA(
  ...
  if (P != 0) {
    UTILS::ComputeRelocation(P, opacity_old.contiguous().data<float>(),
                             scale_old.contiguous().data<float>(),
                             N.contiguous().data<int>(),
                             binoms.contiguous().data<float>(), n_max,
...
void adamUpdate(torch::Tensor& param,
  ...
  ADAM::adamUpdate(
      ...
      visible.contiguous().data<bool>(), lr.contiguous().data<float>(), b1, b2,
      eps, N, M, lr.numel() > 1);
}
